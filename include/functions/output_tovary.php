...
class showContents{
	...
	function showAllUserGrid($sorting = "", $brand = "", $type = "", $start_price = "", $end_price, $check_brand, $name_search="", $sql_nav_menu="") {
		...
		// пошук
		if($name_search != "") {
			$query_search = " `name` LIKE '%$name_search%' AND ";
		} else {
			$name_search = "";
		}
		...
		if($num_rows > 0) {
			...
			// nav sale, liders, news
			if(!empty($sql_nav_menu)){
				Connect::$sql = " SELECT * FROM `tovary` WHERE `visible` = 1 $sql_nav_menu ";
				$query = Connect::$connect->prepare(Connect::$sql);
				$query->execute();
			} else {
				Connect::$sql = " SELECT * FROM `tovary` WHERE $query_search `visible` = '1' $mini_sql $access_check_brand $price_diapaz $main_sort LIMIT ".$records.", $N ";
				$query = Connect::$connect->prepare(Connect::$sql);
				$query->execute();
			}
			...
			if(!empty($name_search)){
				$num_rows_main_query = $query->rowCount();	
			}
		...
		//Пошук	
		if(isset($num_rows_main_query)){
			if($num_rows_main_query == 0) {
				echo "<h1 class='not_found2'>По даному пошуку немає товарів :(</h1>";
				return;
			}	
		}
...
$ext_class->showAllUserGrid($sorting, $brand, $type, $start_price, $end_price, $check_brand, $name_search, $sql_nav_menu);
...
class showContentsList{
	function showAllUserList($sorting = "", $brand, $type, $start_price = "", $end_price, $check_brand, $name_search="", $sql_nav_menu="") {
...
$ext_cls->showAllUserList($sorting, $brand, $type, $start_price, $end_price, $check_brand, $name_search, $sql_nav_menu);
